...
Console.WriteLine("Donnez moi un entier entre 5 et 10 :");

string? val;
int valUser = 0;
bool etatValUser = false;

do
{
   val = Console.ReadLine();

    etatValUser = int.TryParse(val, out valUser);

   if (etatValUser == false)
   {
        Console.WriteLine($"Attention, ce n'est pas une valeur numérique comprise entre 5 et 10. Réessayez :");
        continue;
   } 

   if ((valUser < 5) | (valUser > 10))
   {
        Console.WriteLine($"Attention, la valeur {valUser} n'est pas comprise entre 5 et 10. Réessayez :");
        etatValUser = false;
   } 

} while (etatValUser != true);

Console.WriteLine("Valeur acceptée, merci !");
